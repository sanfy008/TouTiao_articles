...
2. **GPT-6 Astra 自然叙事与抗 AI 腔（Astra Natural Narrative & Anti-Slop Protocol）**：
   - **手机端呼吸感短段落（Mobile Rhythm）**：单段纯汉字控制在 50-80 字，硬性上限 110 字，彻底消灭长段落视觉疲劳。
   - **纯散文自然流（Zero Bullet Points）**：严禁正文出现任何 `-`、`*`、`•` 或 `1.`、`2.` 等列表符号，全篇使用连贯自然的口语化叙事。
   - **拒绝机械对仗与虚假二元对立**：杜绝“这不是...而是...”、“不仅...更是...”、“表面上...实际上...”等典型机器味句式，多用真实动词与具体生活细节。
   - **剔除悬浮套话黑名单**：严禁“在这个快节奏的时代”、“值得深思”、“耐人寻味”、“归根结底”、“说到底”、“总而言之”等假大空修辞。
   - **自然设问取代自问自答**：不使用机械的“那么，我们该怎么做呢？答案是...”，顺着读者日常心理平实展开。
3. **数字逻辑严密闭环（Logical Closure）**：标题若承诺数字（如“三点”、“九个字”），正文论证必须形成严密闭环，严格 1:1 吻合。
4. **真实用户仿真与双轨防线（User Simulation & Resilient Fallbacks）**：ProseMirror 富文本注入、Win32 焦点穿透、AI助手全屏蒙层穿透、封面空插槽自愈、手机预览弹窗原生 JS 穿透，确保 100% 发布成功率。
...
[Phase 2] Stanford STORM 4 视角深度调研 (research_engine.py)
    ...
    │
    ▼
[Phase 3] 深度起草与 Astra 质检审计 (pipeline.py / ContentAuditor)
    │   ├─ 严格字数区间断言：480 ~ 620 纯汉字
    │   ├─ 手机端短段落断言：单段 50-80 字（硬上限 110 纯汉字）
    │   ├─ 纯散文自然叙事断言：零列表项（Zero Bullet Points）
    │   ├─ 机械对仗密度断言：控制“不是...而是”等 AI 腔 ≤ 2 处
    │   ├─ 悬浮套话黑名单过滤：拒绝“值得深思/快节奏/耐人寻味”等
...
| **2. 主笔 (Writer)** | 撰写生活哲理/社会观察短文 | 基于调研事实撰写，控制在 480-620 纯汉字 | 严格遵循 GPT-6 Astra 规范：呼吸感短段落（50-80字）、零 bullet points 纯散文、零机械对仗（不滥用“不是...而是”）、杜绝套话总结，用质朴平常的话讲透现实道理 |
| **3. 审计员 (Auditor)** | 执行 `ContentAuditor` 硬性质检 | 审查字数、段落长度、列表项、AI腔与数字闭环 | 纯汉字 480-620 字；单段 ≤ 110 字；零列表符号；机械对仗 ≤ 2 处；承诺数字逻辑 100% 闭环 |
...
- `[FAIL-005 防线]` **AI 助手蒙层与地理位置隔离**：在选择发文地理位置（如“广州市”）前，必须在 DOM 树中清理掉字节跳动注入的全屏透明遮罩 `.ai-assistant-drawer, .byte-drawer-mask`，防止 Playwright 拦截点击。
- `[FAIL-006 防线]` **GPT-6 Astra 叙事与移动端段落契约**：正文必须为纯净自然流散文，单段纯汉字严格 ≤ 110 字（推荐 50-80 字），严禁在正文中使用任何 markdown 列表符号（`-`, `*`, `1.`），严禁堆砌“不是...而是”机械对仗句式。
