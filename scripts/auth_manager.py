...
def is_page_showing_login(page) -> bool:
    """Check if the page DOM displays an active login dialog, QR code, or form."""
    try:
        if hasattr(page, "evaluate"):
            return bool(
                page.evaluate(
                    ...
                    if (text.includes('扫码登录') || text.includes('验证码登录') || text.includes('手机号登录')) {
                        return true;
                    }
                    const loginEl = document.querySelector('.login-panel, #login-container, [class*="login"], iframe[src*="login"], iframe[src*="sso"]');
                    return Boolean(loginEl);
                }"""
                )
            )
    except Exception:
...
class AuthManager:
    ...
    def is_authenticated(self) -> bool:
        ...
        has_toutiao_cookie = any(
            cookie.get("value")
            and (
                cookie.get("name") in AUTH_LOGIN_COOKIE_NAMES
                or any(
        ...
        # If cookies only contain anonymous visitor cookies without any login credential
        has_login_credential = any(
            cookie.get("name") in AUTH_LOGIN_COOKIE_NAMES for cookie in cookies
        )
        anonymous_only = all(
            cookie.get("name")
            in {
        ...
        if anonymous_only and not has_login_credential:
            return False
    ...
    def setup_auth(
        ...
        try:
            ...
            try:
                ...
                pass
            if hasattr(context, "bring_to_front"):
                try:
                    context.bring_to_front()
                except Exception:
                    pass
            ...
            print("  👉 请在已弹出的 Chrome 窗口中使用今日头条 App 或抖音 App 扫码登录")
            print("  💡 提示：若窗口未自动置顶，请检查 Windows 任务栏 Chrome 图标或按 Alt+Tab 切换至登录窗口。\n")
    ...
    def _find_authenticated_page(self, context: BrowserContext):
        ...
        # 1. If context has cookies API, check for real login credentials
        if hasattr(context, "cookies"):
            try:
                cookies = context.cookies()
                has_login_cookie = any(
                    c.get("name") in AUTH_LOGIN_COOKIE_NAMES for c in cookies
                )
        ...
        # 2. Check tab URLs and exclude pages displaying login UI
        for page in context.pages:
            try:
                if is_authenticated_url(page.url) and not is_page_showing_login(page):
                    return page
            except Exception:
                continue
        return None
    ...
    def _probe_authenticated_session(self, context: BrowserContext):
        ...
        if hasattr(context, "cookies"):
            try:
                cookies = context.cookies()
                if not any(c.get("name") in AUTH_LOGIN_COOKIE_NAMES for c in cookies):
                    return None
            except Exception:
                pass
        try:
