...
def prepare_article(
    ...
    if content:
        if raw:
            ...
            from md2html import strip_frontmatter
            clean_body = strip_frontmatter(content).strip()
            body_lines = clean_body.splitlines()
            if body_lines and body_lines[0].strip().startswith("# "):
                h1_text = body_lines[0].strip()[2:].strip()
                if not normalized_title or h1_text == normalized_title:
                    clean_body = "\n".join(body_lines[1:]).lstrip()

            converted = convert_with_images(clean_body, base_dir=content_base_dir)
...
def publish(
    ...
    if not cover_image_path and not no_cover and content_images:
        cover_image_path = content_images[0]["path"]
        print(f"🖼️ Auto-selected first inline image as cover: {cover_image_path}")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 3.5 Publishing Options (Location, Exclusive/Shoufa, Claims, Ads, Weitoutiao)
            print("⚙️ Configuring publishing options...")
            try:
                page.evaluate("""() => {
                    document.querySelectorAll('.byte-drawer-mask, .ai-assistant-drawer, .byte-modal-mask').forEach(el => el.remove());
                }""")
            except Exception:
                pass

            # 3.5.1 头条首发 (Exclusive)
            try:
                shoufa_input = page.locator("label.checkbox-with-tip input, label:has-text('头条首发') input").first
                if shoufa_input.count() > 0:
                    is_chk = shoufa_input.is_checked()
                    if exclusive and not is_chk:
                        page.locator("label:has-text('头条首发')").first.click(force=True)
                        print("  ✅ '头条首发' checked.")
                    elif not exclusive and is_chk:
                        page.locator("label:has-text('头条首发')").first.click(force=True)
                        print("  ✅ '头条首发' unchecked.")
                    else:
                        print(f"  ✅ '头条首发' status: {is_chk}")
            except Exception as e:
                print(f"  ⚠️ Warning configuring '头条首发': {e}")

            # 3.5.2 添加位置 (Location / City)
            if location:
                try:
            ...
            # 3.5.3 同时发布微头条
            try:
                weitoutiao_input = page.locator("label:has-text('发布得更多收益') input").first
                if weitoutiao_input.count() > 0 and not weitoutiao_input.is_checked():
                    page.locator("label:has-text('发布得更多收益')").first.click(force=True)
                    print("  ✅ '同时发布微头条' checked.")
            except Exception:
                pass

            # 3.5.4 投放广告赚收益
            try:
                ad_input = page.locator("label:has-text('投放广告赚收益') input").first
                if ad_input.count() > 0 and not ad_input.is_checked():
                    page.locator("label:has-text('投放广告赚收益')").first.click(force=True)
                    print("  ✅ '投放广告赚收益' checked.")
            except Exception:
                pass
            ...
            if claim:
                try:
                    claim_input = page.locator(f"label:has-text('{claim}') input").first
                    if claim_input.count() > 0 and not claim_input.is_checked():
                        page.locator(f"label:has-text('{claim}')").first.click(force=True)
                        print(f"  ✅ 作品声明 '{claim}' checked.")
                except Exception as e:
                    print(f"  ⚠️ Warning setting claim '{claim}': {e}")
            ...
            if mode == "auto":
                ...
                try:
                    ...
                    # Strategy: Try specific button locators first
                    initial_btn = (
                        page.locator("button.publish-btn-last")
                        .or_(page.locator("button").filter(has_text="预览并发布"))
                        .or_(page.locator("button").filter(has_text="发布"))
                        .last
                    )
                    ...
                    if initial_btn.is_visible() and initial_btn.is_enabled():
                        ...
                        print("  Attempting JS click on publish button...")
                        page.evaluate("""() => {
                            const btn = Array.from(document.querySelectorAll('button')).find(b => 
                                b.innerText.includes('预览并发布') || b.className.includes('publish-btn-last')
                            );
                            if (btn) btn.click();
                        }""")

                    # Step 4.2: Wait for potential preview/modal
                    print("  Waiting for interface response (6s)...")
                    time.sleep(6)

                    # Step 4.3: Final Confirmation Button ("确认发布" in modal/footer)
                    print("  Step 2: Looking for Final Confirm button ('确认发布')...")
                    confirmed = False
                    for _ in range(5):
                        # Attempt JS click for 确认发布
                        clicked = page.evaluate("""() => {
                            const btn = Array.from(document.querySelectorAll('button')).find(b => 
                                b.innerText.trim() === '确认发布' || b.innerText.includes('确认发布')
                            );
                            if (btn) {
                                btn.click();
                                return true;
                        ...
                        if clicked:
                            print("  ✅ Clicked '确认发布' successfully.")
                            confirmed = True
                            break
                        
                        confirm_btn = (
                            page.locator("button").filter(has_text="确认发布")
                            .or_(page.locator(".byte-modal .byte-btn-primary").filter(has_text="确定"))
                            .or_(page.locator(".byte-modal .byte-btn-primary").filter(has_text="确认发布"))
                            .last
                        )
                        if confirm_btn.is_visible():
                            confirm_btn.click(force=True)
                            print("  ✅ Clicked confirmation button via locator.")
                            confirmed = True
                            break
                        time.sleep(1)

                    if not confirmed:
                        print("  No confirmation modal detected (proceeding to verification).")
                ...
                if headless:
                    ...
                    if wait_seconds is not None:
                        if wait_seconds > 0:
                            print(
                                f"   Waiting {wait_seconds} seconds for visual confirmation..."
                            )
                            time.sleep(wait_seconds)
                        print("\n✅ Manual draft review/preview finished.")
                        return PublishResult(
                            True,
                            "draft_saved",
                            f"Draft confirmed in Toutiao and waited {wait_seconds}s",
                        )
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="In manual mode, wait N seconds after draft is saved before closing (default: block until Ctrl+C)",
    )
    parser.add_argument(
        "--location",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Declare Toutiao Exclusive (头条首发), default: True",
    )
    parser.add_argument(
        "--claim",
        default="个人观点，仅供参考",
        help="Work claim checkbox text (default: '个人观点，仅供参考')",
    )
